...
in {
  config = lib.mkIf cfg.enable {
    ...
    programs.waybar = {
      ...
      in [
        {
          ...
          modules-left = (endLeftWrap ["sway/workspaces"]) 
            ++ (leftWrap ["sway/mode"]);

          modules-center = (rightWrap ["tray"]) 
            ++ (centerWrap ["clock"]) 
            ++ (leftWrap ["disk"])
            ++ (leftWrap ["memory"])
            ++ (leftWrap ["temperature" "cpu"]);

          modules-right = (rightWrap ["network"]) 
      ...
      style = with config.stylix; with config.lib.stylix.colors.withHashtag; ''
